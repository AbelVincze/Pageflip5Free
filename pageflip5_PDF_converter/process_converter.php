<?php 
	/*
		Converter UI API for PDF to Pageflip5 Converter3
...
?><!DOCTYPE html>
<head>
	<title>Pageflip5 - PDF to Pageflip Converter 3</title>
	<meta name="description" content="Pageflip5" />
	<meta name="author" content="Abel Vincze" />
	<meta http-equiv="refresh" content="0;URL=pdf-converter.php">
</head>
<body>
</body>
</html><?php
	...
	function processUpload() {
			...
			if( isset($_FILES["userfile"]) ) {
				...
				if ($_FILES["userfile"]["error"] > 0) {
					errorExit( $_FILES["userfile"]["error"] );
				} else {
	...
	function processGenerateScripts() {
			...
			if( $preview==0 ) {
				...
				$zip->addFile( "files/pageflip5_for_pdf/js/jquery-3.0.0.min.js", "$zname/js/jquery-3.0.0.min.js");
	...
	function generatePageLinkHtml( $links, $page ) {
		...
		for( $i=0; $i<count( $ldata ); $i++ ) {
			...
			$out .= "<div class='pf-hotspot' style='top:".$ldata[$i]["top"]."px;left:".$ldata[$i]["left"]."px;width:".$ldata[$i]["width"]."px;height:".$ldata[$i]["height"]."px;' ";
